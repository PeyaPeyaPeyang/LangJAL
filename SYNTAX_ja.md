...
JAL（Java Assembly Language）は，JVM（Java Virtual
Machine）バイトコードを人間が読みやすいアセンブリ風の構文で記述するためのドメイン固有言語です。JALはJavaクラスファイルへの直接的なマッピングを提供しつつ，ラベルやフィールド参照，メソッド呼び出しなどの高レベルな記法を使って可読性を高めています。
