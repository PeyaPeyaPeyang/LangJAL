...
public class TestSwitchInstructions {
    @Nested
    class TestLookupSwitchCase
            extends AbstractInstructionTestCase<JALParser.JvmInsLookupswitchContext, InstructionEvaluatorLookupSwitch> {
        TestLookupSwitchCase() {
            super(new InstructionEvaluatorLookupSwitch(), EOpcodes.LOOKUPSWITCH);
        }

        @Override
        protected void prepareContext(JALParser.JvmInsLookupswitchContext instruction,
            ...
            for (JALParser.JvmInsArgLookupSwitchCaseContext kase : instruction.jvmInsArgLookupSwitch()
            ...
            return set(
                    of(
                            ...
                            new LookupSwitchInsnNode(
                                    null,
                                    new int[]{1, 5},
                                    new org.objectweb.asm.tree.LabelNode[]{null, null}
                            )
                    ),
                    of(
                            create(integerValue(), integerValue()).expected(create(integerValue())),
                            "lookupswitch { 1:L1, 5:L2, default:L3 }",
                            new LookupSwitchInsnNode(
                                    null,
                                    new int[]{1, 5},
    ...
    @Nested
    class TestTableSwitchCase
            extends AbstractInstructionTestCase<JALParser.JvmInsTableswitchContext, InstructionEvaluatorTableSwitch> {
        TestTableSwitchCase() {
            super(new InstructionEvaluatorTableSwitch(), EOpcodes.TABLESWITCH);
        }

        @Override
        protected void prepareContext(JALParser.JvmInsTableswitchContext instruction,
                                      org.objectweb.asm.tree.ClassNode ownerClass,
                                      org.objectweb.asm.tree.MethodNode ownerMethod, InstructionsHolder instructions,
                                      LabelsHolder labels, LocalVariablesHolder locals) {
            for (JALParser.LabelNameContext labelName : instruction.jvmInsArgTableSwitch()
            ...
            return set(
                    of(
                            ...
                            new TableSwitchInsnNode(
                                    ...
                                    5,
                                    null,
                                    new org.objectweb.asm.tree.LabelNode[]{null, null, null}
                            )
                    ),
                    of(
                            create(integerValue(), integerValue()).expected(create(integerValue())),
